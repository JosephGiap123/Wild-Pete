...
public abstract class BasePlayerMovement2D : MonoBehaviour
{
    ...
    [SerializeField] protected AttackHitbox hitboxManager; // Can be null if not assigned
    [SerializeField] protected AnimScript animatorScript; // Can be null if not assigned
    [SerializeField] protected InteractionDetection interactor; // Can be null if not assigned
    ...
    protected virtual void Update()
    {
        ...
        // Don't process input or movement if dead
        if (isDead) return;
    ...
    protected virtual void HandleInput()
    {
        ...
        if (Input.GetKeyDown(KeyCode.T) && isGrounded && !isCrouching && !isDashing && !isAttacking && !isReloading && ammoCount < maxAmmo && (PlayerInventory.instance == null || PlayerInventory.instance.HasItem("Ammo") > 0))
        {
    ...
    {
        if (animatorScript == null) return; // Skip if animator not assigned
    ...
    public virtual void HurtPlayer(int damage, float knockbackDirection, Vector2 knockbackForce)
    {
        ...
        if (damageText != null)
        {
            ...
            DamageText damageTextComponent = dmgText.GetComponentInChildren<DamageText>();
            if (damageTextComponent != null)
                damageTextComponent.Initialize(new(knockbackForce.x * knockbackDirection, 5f), damage, Color.red, Color.black);
        }
    ...
    protected virtual IEnumerator DeathSequence()
    {
        ...
        // The RespawnManager will handle showing the respawn UI
        OnDeathAnimationComplete();
    }

    protected virtual void OnDeathAnimationComplete()
    {
        // Don't disable the player - let RespawnManager handle respawning
        // The player will stay active but disabled through movement controls
        // gameObject.SetActive(false); // Removed - respawn system handles this
    }

    public virtual void Respawn()
    {
        Debug.Log("BasePlayerMovement2D: Respawn() called - resetting all state");
        
        // Stop all coroutines FIRST
        StopAllCoroutines();
        
        // Reset all player state flags
        isDead = false;
        isHurt = false;
        isInvincible = false;
        isAttacking = false;
        ...
        // Reset collider states if needed
        if (boxCol != null)
        {
            boxCol.enabled = true;
        ...
        // Give brief invincibility after respawn (3 seconds) to prevent immediate re-death
        StartCoroutine(RespawnInvincibility(3f));

        Debug.Log($"BasePlayerMovement2D: Respawn complete - isDead: {isDead}, enabled: {enabled}, active: {gameObject.activeSelf}");
    }

    protected virtual IEnumerator RespawnInvincibility(float duration)
    {
        isInvincible = true;
